...
- **App admin** (`/admin`, signed-in app admins only) — every cloud league with its owner (name + email), **Open** any league as a league admin without joining, and **Members** to approve/reject/remove people, promote/demote league admins, or transfer owner. Super admin also gets **Operators** (`/admin/operators`) to promote signed-in users to app admin or kick them. Super admin itself is granted only from the Firebase console. Hidden in local-only mode
